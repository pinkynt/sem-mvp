...
type Step = "time" | "payment" | "confirm" | "done";

type ResultState =
  | { kind: "qr"; paymentId: string; qr: ParkingQrDto; status: ParkingPaymentStatusDto | null }
  | { kind: "paid"; receipt: ParkingReceiptDto; method: "efectivo" | "digital" };

const FLOW: Step[] = ["time", "payment", "confirm"];
...
export default function PrepagoChargePage() {
  ...
  // QR polling — confirm the prepago payment without leaving this screen.
  useEffect(() => {
    ...
    const interval = window.setInterval(async () => {
      try {
        ...
        if (status.receipt) {
          setResult({ kind: "paid", receipt: status.receipt, method: "digital" });
        }
      } catch {
  ...
  async function confirmCharge() {
    if (!session || !payment || !duration) return;
    setSubmitting(true);
    setError(null);
    try {
      const method: PaymentMethod = payment === "efectivo" ? "cash" : "digital";
      const response = await createParkingPayment({
  ...
  return (
    <main className="flex h-dvh w-full flex-col overflow-hidden bg-surface">
      ...
      {step !== "done" && (
        <div className="flex items-center gap-2 border-b border-border px-4 py-3">
          ...
          <span className="text-sm font-semibold text-ink-soft">
            Cobro prepago · Paso {stepNumber} de {FLOW.length}
          </span>
        </div>
      )}
      ...
      >
            ...
            <div className="flex flex-col gap-3">
              {DURATIONS.map((opt) => (
                <ChoiceButton
                  ...
                  onClick={() => { setTimeId(opt.id); setStep("payment"); }}
                />
              ))}
            </div>
          </>
        ...
        {step === "confirm" && quote && (
          <>
            <h2 className="text-2xl font-extrabold tracking-tight text-ink">Confirmar cobro</h2>
            ...
            <div className="mt-auto">
              <Button fullWidth disabled={submitting} onClick={() => void confirmCharge()}>
        ...
        {step === "done" && result?.kind === "paid" && (
          <PaidResult
            receipt={result.receipt}
            method={result.method}
            onDone={() => router.push("/permisionario")}
          />
        )}
      </div>
    </main>
  ...
  return (
    <div className="flex flex-col items-center gap-7 rounded-card border border-border bg-surface px-6 py-9 text-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <h2 className="text-3xl font-extrabold tracking-tight text-brand-strong">Cobro registrado</h2>
      </div>
      ...
      <dl className="flex w-full max-w-xs flex-col gap-3 border-t border-border pt-6">
        ...
        {receipt.payment.validUntil && (
          <SummaryRow term="Válido hasta" value={timeLabel(receipt.payment.validUntil)} />
        )}
      ...
      <Button variant="confirm" fullWidth onClick={onDone}>
        Listo
      </Button>
    </div>
  );
