...
const columns: ColumnDef<PermitHolderAdminDto>[] = [
  ...
  { id: "account", header: "Cuenta futura", cell: ({ row }) => row.original.account ? <DashboardBadge tone={row.original.account.active ? "digital" : "neutral"}>{row.original.account.username}</DashboardBadge> : "Sin cuenta" },
];
  ...
  return (
    <section className="rounded-card border border-border bg-surface p-5 shadow-sm" aria-label={`Editar ${holder.displayName}`}>
      ...
      <form action={submit} className="mt-5 grid gap-4 md:grid-cols-2">
        <Field name="displayName" label="Nombre" defaultValue={holder.displayName} />
        <Field name="fileNumber" label="Legajo" defaultValue={holder.fileNumber} />
        <label className="block text-sm font-bold text-ink">Zona<select name="zoneId" defaultValue={holder.zoneId} required className="mt-2 min-h-12 w-full rounded-input border border-border px-4">{zones.map((zone) => <option key={zone.id} value={zone.id}>{zone.name}</option>)}</select></label>
        <label className="block text-sm font-bold text-ink">Estado<select name="active" defaultValue={String(holder.active)} className="mt-2 min-h-12 w-full rounded-input border border-border px-4"><option value="true">Activo</option><option value="false">Inactivo</option></select></label>
        <Field name="username" label="Usuario futuro" defaultValue={holder.account?.username ?? ""} />
        <label className="block text-sm font-bold text-ink">Cuenta futura<select name="accountActive" defaultValue={String(holder.account?.active ?? true)} className="mt-2 min-h-12 w-full rounded-input border border-border px-4"><option value="true">Activa</option><option value="false">Inactiva</option></select></label>
        <div className="md:col-span-2"><Field id={passwordId} name="password" label={holder.account ? "Nueva contraseña (opcional)" : "Contraseña inicial"} type="text" /><div className="mt-2"><PasswordGenerateButton targetId={passwordId} /></div><p className="mt-2 text-xs text-ink-soft">Al guardar, el servidor reemplaza el valor por un hash scrypt. Nunca se almacena texto plano.</p></div>
      ...
      {holder.account && <form action={submit} className="mt-5 rounded-input bg-brand-tint/50 p-4"><input type="hidden" name="displayName" value={holder.displayName} /><input type="hidden" name="fileNumber" value={holder.fileNumber} /><input type="hidden" name="zoneId" value={holder.zoneId} /><input type="hidden" name="active" value={String(holder.active)} /><input type="hidden" name="username" value={holder.account.username} /><input type="hidden" name="accountActive" value={String(holder.account.active)} /><Field id={resetPasswordId} name="password" label="Resetear contraseña" type="text" /><div className="mt-3 flex flex-wrap gap-2"><PasswordGenerateButton targetId={resetPasswordId} /><button className="rounded-pill bg-brand px-5 py-2 text-sm font-bold text-white" type="submit">Guardar nuevo hash</button></div></form>}
    </section>
  );
}
