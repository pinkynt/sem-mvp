...
export async function POST(request: Request) { try { const user = await requireDashboardUser(); return Response.json(await upsertPermitHolder(await request.json(), user.id), { status: 201 }); } catch (error) { return dashboardApiError(error); } }
