...
export async function POST(request: Request) { try { await requireDashboardUser(); return Response.json(await createTariff(await request.json()), { status: 201 }); } catch (error) { return dashboardApiError(error, "No se pudo crear la tarifa. Revisá los datos ingresados e intentá nuevamente."); } }
