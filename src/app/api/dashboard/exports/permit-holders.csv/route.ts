...
export async function GET() { try { await requireDashboardUser(); const rows = await getPermitHolders(); return csvResponse("permisionarios-sem.csv", toCsv(["Nombre", "Legajo", "Zona", "Activo", "Usuario futuro", "Cuenta activa"], rows.map((row) => [row.displayName, row.fileNumber, row.zoneName, row.active ? "sí" : "no", row.account?.username ?? "", row.account?.active ? "sí" : "no"]))); } catch (error) { return dashboardApiError(error); } }
